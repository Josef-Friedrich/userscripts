// Origin: https://github.com/otringal/MB-userscripts/blob/master/Musicbrainz_acoustid.user.js
...
css.innerHTML =
  "td > a[href^='//acoustid.org/track/'] > code {display: inline-block; white-space: nowrap; overflow-x: hidden; width: " +
  numCharacters +
  'ch}'
css.innerHTML +=
  '.hidelist, .hidelist + br {display: none;} .showids span {white-space: nowrap; margin: 0.4em 0em; padding: 0.1em 0.3em; font-size: smaller; text-transform: uppercase; font-weight: 600; background-color: rgba(250, 200, 35, 0.5); cursor: pointer;}'
        ...
        for (var b = 0; b < json.mbids.length; b++) {
          if (json.mbids[b].mbid == mbidtocheck) {
            $.each(json.mbids[b].tracks, function () {
              var a = $(
                '<a href="#"><img src="//acoustid.org/static/acoustid-wave-12.png" title="' +
                  this.id +
                  '" alt="AcoustID" /></a>'
              )
...
function updateMergeOrEdits (check, path) {
  ...
  if (path.match(/edit/) || path.match(/votes/))
    $(
      '.details.merge-recordings thead tr th:nth-child(' + (3 - numb) + ')'
    ).after('<th>AcoustIDs</th>')
  else if (path.match(/recording\/merge/))
    $('.tbl thead tr th:nth-child(' + (3 - numb) + ')').after(
      '<th>AcoustIDs</th>'
    )
  ...
  findAcoustIDsByMBIDs(mbids, function (json) {
    ...
    $('' + classPath + '.tbl tr td:nth-child(' + (2 - numb) + ')').each(
      function (i, td) {
        //for each recording get mbid
          ...
          for (var b = 0; b < json.mbids.length; b++) {
            if (json.mbids[b].mbid == mbidtocheck) {
              ...
              if (
                addShowHideButton &&
                alwaysShowIds != 0 &&
                json.mbids[b].tracks.length > alwaysShowIds
              ) {
                ...
                newtd +=
                  "<div class='showids allids'><span>show all (+" +
                  hiddenlength +
                  ')</span></div>'
                showhide()
                break
...
function updatePages (path) {
  if (
    enableMiniIcons &&
    (path.match(/artist\/[A-Fa-f0-9-]+\/recordings/) ||
      path.match(/release\/[A-Fa-f0-9-]+$/))
  ) {
    updateArtistRecordingsPage()
    return
  } else if (
    enableAcoustList &&
    (path.match(/recording\/merge/) ||
      path.match(/edit/) ||
      path.match(/votes/))
  ) {
